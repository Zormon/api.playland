...
app()->group('monitor', ['middleware' => 'auth.required', function () {
    ...
    app()->get('/participation/(\d+)/(\d+)(/-?\d+)?', ['middleware' => 'is:admin|taquilla|monitor', 'MonitorController@canParticipate']);

    // Register a new participation
    app()->post('/participation/(\d+)/(\d+)(/-?\d+)?', ['middleware' => 'is:admin|taquilla|monitor', 'MonitorController@registerParticipation']);
}]);
