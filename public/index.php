...
|--------------------------------------------------------------------------
*/
auth()->middleware('auth.required', function() { response()->plain(null, 401); });
auth()->middleware('auth.guest',    function() { response()->plain(null, 401); });
...
*/
auth()->createRoles([
    'admin' => ['users:viewall', 'users:edit', 'users:profile'],
    'adulto' => ['users:viewself'],
    'guest' => [],
]);
